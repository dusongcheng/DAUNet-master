import torch
from architecture.MST_model3 import ModelNet
import argparse
...
def record_loss(loss_csv,epoch, l1, rmse, psnr, ssim):
    """ Record many results."""
    loss_csv.write('{},{},{},{},{}\n'.format(epoch, l1, rmse, psnr, ssim))
    loss_csv.flush()    
    loss_csv.close
...
if __name__ == '__main__':
    ...
    for epoch in (range(opt.n_epochs)):
        ...
        l1_log = []
        rmse_log = []
        for msi, hsi_g, src in tqdm(Hyper_train):
            msi = msi.cuda()
            hsi_g = hsi_g.cuda()
            src = src.cuda()
        ...
        if (epoch>600) or (epoch % 20 == 0):
            with torch.no_grad():
                for (msi, hsi_g, src) in tqdm(Hyper_test):
                    spenet.eval()
                    msi = msi.cuda()
                    hsi_g = hsi_g.cuda()
                    msi = Variable(msi)
                    hsi_g = Variable(hsi_g)
                    src = src.cuda()
                    src = torch.repeat_interleave(src, repeats=1, dim=0)
                    fout = spenet(msi, src)
                    msi_ = (msi).detach().cpu().numpy()
                    hsi_g_ = (hsi_g).detach().cpu().numpy()
                    fout_ = (fout).detach().cpu().numpy()
                    for i in range(31):
                        psnr_g.append(compare_psnr(hsi_g_[0,i,:,:],fout_[0,i,:,:], data_range=1.))
                    ...
                    l1 = compute_L1(fout_, hsi_g_)
                    l1_log.append(l1)
                    rmse = compute_rmse(fout_, hsi_g_)
                    rmse_log.append(rmse)
            ssim_ = np.mean(np.array(ssim_log))
            psnr_ = np.mean(np.array(psnr_g))
            l1_ = np.mean(np.array(l1_log))
            rmse_ = np.mean(np.array(rmse_log))
            # if opt.local_rank == 0:
                # PSNR_logger.log(epoch,psnr_)
                # Losst_logger.log(epoch,np.mean(np.array(loss_t)))
                # Lossm_logger.log(epoch,ssim_)
            record_loss(loss_csv, epoch, l1_, rmse_, psnr_, ssim_)
            print("epoch:%d, train_loss:%f, L1:%f, rmse:%f, psnr:%f, ssim:%f"%(epoch, loss_[-1], l1_, rmse_, psnr_, ssim_))
            if ssim_ > 0.999 and epoch > 0:
                # ssim_best = ssim_
                save_model(epoch,spenet)
            if psnr_ > 41.50 and epoch > 0:
                # psnr_best = psnr_
                save_model(epoch,spenet)
