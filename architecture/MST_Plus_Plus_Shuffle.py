...
class MS_MSA(nn.Module):
    def __init__(
        ...
        self.to_q = nn.Linear(dim * 4, dim_head * 4 * heads, bias=False)
        self.to_k = nn.Linear(dim * 4, dim_head * 4 * heads, bias=False)
        self.to_v = nn.Linear(dim * 4, dim_head * 4 * heads, bias=False)
        self.rescale = nn.Parameter(torch.ones(heads, 1, 1))
        self.proj = nn.Linear(dim_head * heads * 4, dim * 4, bias=True)
    ...
    def forward(self, x_in):
        ...
        return out: [b,h,w,c]
        """
        x_res = x_in.permute(0, 3, 1, 2)
        x = pixel_unshuffle(x_res, 2)
        x = x.permute(0, 2, 3, 1)
        b, h, w, c = x.shape
        ...
        x = x.reshape(b, h * w, self.num_heads * self.dim_head * 4)
        out_c = self.proj(x).view(b, h, w, c)
        out_c = out_c.permute(0, 3, 1, 2)
        out_c = F.pixel_shuffle(out_c, 2)
        out_c = out_c.permute(0, 2, 3, 1)
        out_p = self.pos_emb(x_res).permute(0, 2, 3, 1)
        out = out_c + out_p
...
class MST_Plus_Plus_Shuffle(nn.Module):
    def __init__(self, in_channels=3, out_channels=31, n_feat=31, stage=3):
        super(MST_Plus_Plus_Shuffle, self).__init__()
        self.stage = stage
